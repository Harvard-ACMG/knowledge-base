...
`$OMP\_NUM\_THREADS` will be set automatically for you when you source one of the [GEOS-Chem environment files](cannon-environment-files.md).  This will set `$OMP\_NUM\_THREADS` to the same number of CPUs that you requested in your
interactive session.**

If for some reason you wanted to change the value of `$OMP\_NUM\_THREADS` within an interactive session, simply type:
